...
Hay algunas celdas que contienen varias ideas separadas por comas, otras las separan con diagonales. Asumiré que esas listas tratan de varios "concepto" distintos, así que los separaré.

```{r conceptos, echo=FALSE, include=TRUE}

# Datos generales de las hojas
hojas <- names(concept_hojas)
hojas <- hojas[!grepl("Introducción", hojas)]
...
```{r json, echo=FALSE, include=TRUE, eval=FALSE}

top_level_val <- unique(conceptos$tema)
nested_df <- unique(conceptos[c("tema", "concepto", "id_p", "id")])
c("children", "name", "info.x", "percentage.x")

output <- lapply(top_level_val, function(root) 
  {
    root_lst <- list(name = root)
    
    root_lst$tema <- lapply(1:nrow(nested_df), function(i) 
      {
       chld_mrg <- merge(nested_df[nested_df$tema == nested_df$tema[i],], conceptos)
      
       parent_lst <- list(name = nested_df$tema[i][1],
                          tooltip = nested_df$id_p[i][1],
                          percentage = nested_df$id[i][1]
      )
       parent_lst$children <- lapply(1:nrow(chld_mrg), function(j) 
         list(name = conceptos$tema.y[j][1],
              tooltip = conceptos$id_p.y[j][1],
              percentage = conceptos$id.y[j][1]
              )
      )
       return(parent_lst)
    })
